...
- [ ] Two-key negative cases: `bun run demo --slack` with the env var unset or invalid dies at boot (exit 1) with a message naming the flag and the var — and never echoing the URL value; `bun run demo` without the flag ignores the env var entirely.
